...
class Name implements java.io.Serializable {
	...
	private Object consumeNextObjectField(
		...
		/*
		 * can.senturk: i'm including here the component map, so that
		 * string key maps will be able to be referenced by name
		 */
		if (evalBaseObject instanceof Map) {
			//&& ((Map) evalBaseObject).containsKey(varName)) {
			Object obj = ((Map) evalBaseObject).get(varName);
			if (obj == null) {
//				this.lastEvalName = varName;
//				this.evalName = suffix(evalName);
//				this.evalBaseObject = "null";
				return completeRound(evalName, FINISHED, Primitive.NULL);
				//return new Variable(varName, null, new Modifiers());
			} else {
				return completeRound(varName, suffix(evalName), obj);
			}
		}
